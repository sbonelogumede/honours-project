...
for(i in 1:length(horizon)){
	m <- horizon[i]
	x <- 1 : n
	y <- na.locf(na.approx(train$NO2[1:n], na.rm=F), fromLast=T) 
	x_predict <- (n + 1) : (n + m)
	y_predict <- na.locf(na.approx(test$NO2[1:m], na.rm=T), fromLast=T)
	
	truth <- list(x = c(x, x_predict), f = c(y, y_predict))
	data <- list(N_obs=length(x), x_obs=x, y_obs=y, N_predict=m, x_predict=x_predict)
	
	opt_fit <- optimizing(gp_opt1, data=data, seed=5838298, hessian=F)
	
	pred_data <- list(alpha=opt_fit$par[2], rho=opt_fit$par[1], sigma=opt_fit$par[3],
