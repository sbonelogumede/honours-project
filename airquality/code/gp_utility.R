...
plot_gp_post_realizations <- function(fit, data, true, title) {
  ...
  xlim <- range(c(data$x_obs, data$x_predict, true$x))
  ylim <- range(c(data$y_obs, samples$f_predict, true$f), na.rm = TRUE)
...
plot_gp_prior_quantiles <- function(fit, xs, title) {
  ...
  plot(1, type="n", main=title,
       xlab="Time", ylab="f", xlim=xlim, ylim=xlim)
...
plot_gp_post_quantiles <- function(fit, data, true, title) {
  ...
  plot(1, type="n", main=title,
       xlab="Time", ylab="f", xlim=xlim, ylim=ylim)
