functions {
  ...
  matrix periodic_kernel(vector t1, vector t2, real alpha, real rho, real T) {
    int n1 = rows(t1);
    int n2 = rows(t2);
    matrix[n1, n2] K;
    ...
    for (i in 1:n1) {
      for (j in 1:n2) {
        // Compute periodic distance for scalar time values
        real diff = t1[i] - t2[j];
        real sin_val = sin(pi() * diff / T);
        K[i, j] = square(alpha) * exp(-2 * square(sin_val) / square(rho));
      }
    }
    return K;
  }
...
model {
  ...
  rho ~ inv_gamma(5, 50);       // Lengthscale prior
  ...
  matrix[n1, n1] K11 = periodic_kernel(X1_time, X1_time, alpha, rho, T)
...
generated quantities {
  ...
  
  {
    ...
    matrix[n1, n1] K11 = periodic_kernel(X1_time, X1_time, alpha, rho, T)
                         + diag_matrix(rep_vector(square(sigma), n1));
    matrix[n1, n2] K12 = periodic_kernel(X1_time, X2_time, alpha, rho, T);
    matrix[n2, n2] K22 = periodic_kernel(X2_time, X2_time, alpha, rho, T)
