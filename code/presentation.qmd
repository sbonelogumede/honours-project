...
# Function to simulate Gaussian Process and plot interactively
gp_interactive_plot <- function() {
  
  # Create a basic set of data
  set.seed(123)
  x_train <- seq(1, 100, by = 10)  # Initial training points
  y_train <- sin(x_train / 10) + rnorm(length(x_train), sd = 0.2)  # Noisy observations
  
  # Create the initial GP model
  kernel <- rbfdot(sigma = 0.1)  # Radial basis function kernel
  gp_model <- gausspr(x_train, y_train, kernel = kernel)
  
  # Generate test data for predictions
  x_test <- seq(1, 100, length.out = 500)
  
  # Create a plotly plot
  plot_ly() %>%
    add_lines(x = x_test, y = predict(gp_model, x_test), name = "Mean", line = list(color = 'black')) %>%
    add_ribbons(x = x_test, ymin = predict(gp_model, x_test) - 1, ymax = predict(gp_model, x_test) + 1, name = "Uncertainty", fillcolor = 'rgba(0, 0, 255, 0.2)') %>%
    add_markers(x = x_train, y = y_train, mode = 'markers', name = "Data Points", marker = list(color = 'red', size = 10)) %>%
    layout(title = "Gaussian Process Regression", 
           xaxis = list(title = "x"), 
           yaxis = list(title = "y"))
}

# Run a Shiny app for interactive plotting
shinyApp(
  ui = fluidPage(
    titlePanel("Interactive Gaussian Process Plot"),
    sidebarLayout(
      sidebarPanel(
        actionButton("update", "Update GP with New Data", class = "btn-primary")
      ),
      mainPanel(
        plotlyOutput("gp_plot")
      )
    )
  ),
  ...
  server = function(input, output, session) {
    ...
    # Reactive values to store training data
    data <- reactiveValues(x_train = seq(1, 100, by = 10),
                           y_train = sin(seq(1, 100, by = 10) / 10) + rnorm(10, sd = 0.2))
    ...
    observeEvent(input$update, {
      new_x <- max(data$x_train) + 10
      new_y <- sin(new_x / 10) + rnorm(1, sd = 0.2)
      
      # Add new data point
      data$x_train <- c(data$x_train, new_x)
      data$y_train <- c(data$y_train, new_y)
      
      # Re-render plot with updated GP
      output$gp_plot <- renderPlotly({
        gp_interactive_plot()
      })
    })
...
## Common Kernels used in Gaussian Processes

:::{.no-caption}
![](kernels.png){width=1500px height=500px}
:::
