...
$$
k(x_{i}, x_{j}) = \sigma^2(x_{i} - c)(x_{j} - c)
$$
...
\text{where, } \alpha \; \text{controls the marginal variability} \; \text{and}\\
\rho \;
\text{controls how quickly the correlations between function values decay.}
$$

#### Generalization of the squared exponential

$$
k(x_{i}, x_{j}) = \frac{2^{1-v}}{\Gamma(v)}(\sqrt{2v} \frac{|x_{i} - x_{j}|}{\rho})^{v}K_{v}(\sqrt{2v} \frac{|x_{i} - x_{j}|}{\rho}),\\
\text{where} \; K_{ν} \; \text{is a modified Bessel function of the second kind.}
$$
