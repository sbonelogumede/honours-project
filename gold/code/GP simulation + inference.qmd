...
```
GP prior draws (before conditioning)

```{r}
library(ggplot2)
library(mvtnorm)

# PRIOR: draw functions from GP prior with true parameters
set.seed(42)
n_prior <- 10
f_prior_draws <- rmvnorm(n_prior, mean = rep(0, Ngrid), sigma = Kxx)

df_prior <- data.frame(
  x = rep(xgrid, n_prior),
  f = as.vector(t(f_prior_draws)),
  draw = factor(rep(1:n_prior, each = Ngrid))
)

ggplot(df_prior, aes(x, f, group = draw)) +
...
```
GP Posterior predictive plot (after conditioning on data):
-posterior mean
-credible interval
-observed data 
```{r}
...
# Summaries for each observed x
pred_mean <- apply(yrep, 2, mean)
pred_lo   <- apply(yrep, 2, quantile, 0.05)
pred_hi   <- apply(yrep, 2, quantile, 0.95)

df_pred <- data.frame(x = x_obs, mean = pred_mean, lo = pred_lo, hi = pred_hi)
df_obs  <- data.frame(x = x_obs, y = y_obs)

# Posterior predictive check (credible band + observations)
ggplot() +
  geom_ribbon(data = df_pred, aes(x = x, ymin = lo, ymax = hi),
              fill = "lightblue", alpha = 0.5) +
  geom_line(data = df_pred, aes(x = x, y = mean), color = "blue", size = 1) +
  geom_point(data = df_obs, aes(x = x, y = y), color = "black", size = 2) +
  labs(title = "Posterior Predictive Distribution (Observed Inputs)",
       x = "x", y = "y") +
  theme_minimal()
